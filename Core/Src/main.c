...
void StartDefaultTask(void *argument) {
	...
	for (;;) {
		...
		// 2. НОВА ЛОГІКА ОПИТУВАННЯ ОДНІЄЇ КНОПКИ (Мультіклік)
		// ---------------------------------------------------------
		// Заміни BTN_GPIO_Port та BTN_Pin на твій реальний пін (наприклад, BTN_NEXT_Pin)
		...
		// Відкриваємо новий файл після того, як AudioTask закрив попередній
