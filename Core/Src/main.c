...
int main(void) {
	...
	while (1) {
		// Дозавантаження даних з великим запасом (якщо менше одного повного фрейму)

		// Декодуємо свіжу порцію туди, звідки DMA щойно забрав дані
	...
	if (res == FR_OK) {
		while (1) {
			...
			if (res != FR_OK || fno.fname[0] == 0)
				break; // Кінець директорії

			if (!(fno.fattrib & AM_DIR)) { // Якщо це файл, а не папка
				// Перевіряємо, чи закінчується на .mp3 (регістронезалежно)
...
void StartDefaultTask(void *argument) {
	...
	osDelay(100); // ЗАМІНА: osDelay замість HAL_Delay
	...
	if (total_songs > 0) {
		...
		// Виводимо плейлист на OLED
		SSD1306_Clear();
		SSD1306_GotoXY(0, 0);
		SSD1306_Puts("--- PLAYLIST ---", &Font_7x10, SSD1306_COLOR_WHITE);

		int max_lines_to_show = (total_songs < 5) ? total_songs : 5;

		for (int i = 0; i < max_lines_to_show; i++) {
			int y_position = 12 + (i * 11);
			...
			SSD1306_Puts(playlist[i], &Font_7x10, SSD1306_COLOR_WHITE);
		}

		SSD1306_UpdateScreen();
		osDelay(10); // ЗАМІНА: osDelay замість HAL_Delay
...
//		          adc_volume = HAL_ADC_GetValue(&hadc1); // Отримуємо значення від 0 до 4095
//		      }
		adc_volume = 500;
		// 2. ОПИТУВАННЯ КНОПОК
		// Якщо кнопка натиснута (замикає на GND = RESET) і файл грає
		if (HAL_GPIO_ReadPin(BTN_NEXT_GPIO_Port, BTN_NEXT_Pin) == GPIO_PIN_SET
				&& file_opened == 1) {
			track_direction = 1; // Вперед
			skip_track = 1;      // Даємо команду аудіо-таску
			printf("Next");
			osDelay(300);        // Антибрязкіт
		} else if (HAL_GPIO_ReadPin(BTN_PREV_GPIO_Port, BTN_PREV_Pin)
				== GPIO_PIN_SET && file_opened == 1) {
			track_direction = -1; // Назад
			skip_track = 1;
			printf("Prev");
			osDelay(300);
		...
		// 3. ПЕРЕМИКАННЯ ФАЙЛІВ (ЛОГІКА ЕКРАНУ)
		// Спрацьовує і при природньому кінці пісні, і при натисканні кнопки
		if (file_opened == 0 && total_songs > 0) {
			...
			int max_lines = (total_songs < 5) ? total_songs : 5;
			for (int i = 0; i < max_lines; i++) {
				SSD1306_GotoXY(0, 12 + (i * 11));
				if (i == current_song_index) {
					SSD1306_Puts("> ", &Font_7x10, SSD1306_COLOR_WHITE);
				} else {
					SSD1306_Puts("  ", &Font_7x10, SSD1306_COLOR_WHITE);
...
void StartAudioTask(void *argument) {
	...
	for (;;) {
		...
		if (bytesLeft < 2048) {
			...
			readPtr = readBuf;
			UINT br = 0;
			f_read(&fil, readBuf + bytesLeft, READ_BUF_SIZE - bytesLeft, &br);
			bytesLeft += br;
		...
		if (osSemaphoreAcquire(dmaSemHandle, osWaitForever) == osOK) {
			if (dma_half_ready || dma_full_ready) {
				int16_t *target_pcm =
						dma_half_ready ? pcmBuf : (pcmBuf + PCM_BUF_SIZE / 2);
				int frame_decoded = 0;
				...
				while (!frame_decoded && bytesLeft > 0) {
					...
					if (offset >= 0) {
						readPtr += offset;
						bytesLeft -= offset;
						int err = MP3Decode(hMP3Decoder, &readPtr, &bytesLeft,
								target_pcm, 0);
						...
						if (err == ERR_MP3_NONE) {
							...
							if (info.outputSamps < (PCM_BUF_SIZE / 2)) {
								memset(target_pcm + info.outputSamps, 0,
										((PCM_BUF_SIZE / 2) - info.outputSamps)
												* sizeof(int16_t));
							}
						} else {
							readPtr++;
				...
				if (!frame_decoded) {
					memset(target_pcm, 0, (PCM_BUF_SIZE / 2) * sizeof(int16_t));
				}
